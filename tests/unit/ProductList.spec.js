...
describe('렌더링 테스트', () => {
  ...
  test('음료명 한글 렌더링', () => {
    ...
    expect(productNames.at(0)).toBe('카라멜 마끼아또');
    expect(productNames.at(1)).toBe('카푸치노');
    expect(productNames.at(2)).toBe('슈크림 라떼');
  });

  test('음료명 영문 렌더링', () => {
    const productEngNames = wrapper.findAll('[data-test="product-eng-name"');

    expect(productEngNames.at(0)).toBe('Caramel Macchiato');
    expect(productEngNames.at(1)).toBe('Cappuccino');
    expect(productEngNames.at(2)).toBe('Choux Cream Latte');
  });

  test('가격 렌더링', () => {
    const productPrices = wrapper.findAll('[data-test="product-price"');

    expect(productPrices.at(0)).toBe('5,000원');
    expect(productPrices.at(1)).toBe('4,500원');
    expect(productPrices.at(2)).toBe('5,500원');
  });

  test('위첨자 렌더링', () => {
    const productSuperscripts = wrapper.findAll('[data-test="product-superscript"');

    expect(productSuperscripts.at(0)).toBe('Best');
    expect(productSuperscripts.at(1)).toBe('New');
  });
});
